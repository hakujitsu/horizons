...
def ranking_articles(article_arr):
    ...
    # Step 2: If there are still at least 2 articles and the diff in the recommedation scores between them is relatively low, timestamp is also considered as a factor
...
def get_final_recommendations(user, read_article, possible_articles_arr):
    ...
    read_article_headline = read_article.get_headline() # TODO @MY need this method
    read_article_body_text = read_article.get_body_text() # TODO @MY need this method
    ...
    if (len(possible_articles_arr) > 15):
        ...
        for rec_article in possible_articles_arr:
            rec_article_headline = rec_article.get_headline() # TODO @MY need this method
    ...
    for (rec_article, headline_sim_score) in possible_articles_arr:
        rec_article_source = rec_article.get_source() # TODO @MY need this method
        rec_article_body_text = rec_article.get_body_text() # TODO @MY need this method
        timestamp = rec_article.get_timestamp() # TODO @MY need this method (if we want to scrape this)
    ...
    return ranking_articles(top_3_rec_articles)
